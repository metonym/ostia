...
export async function bundleDts(opts: {
  ...
  const { root, source, outFile } = opts
  const srcRoot = join(root, "src")
  const emitted = emitDeclarations([source], srcRoot)
...
function sourceToDtsPath(
  ...
  const exact = norm(source.replace(TS_EXT_RE, ".d.ts"))
  ...
  throw new Error(
    `no declaration emit for ${source} (expected ${exact})\nhave: ${[...emitted.keys()].join("\n")}`,
  )
}
